...
public class JanelaConfirmarAlterarDados1 extends javax.swing.JFrame {
    ...
    int k;
    private List<MyLabel> jLabelDDD;
    private List<MyTextField> jTextFieldDDD;
    private List<MyLabel> jLabelTelefone;
    private List<MyTextField> jTextFieldTelefone;
    private List<MyButton> jButtonDDDTelefones;

    private List<MyLabel> jLabelEmail;
    private List<MyTextField> jTextFieldEmail;
    private List<MyButton> jButtonEmailAS;
    ...
    private void initComponents() throws FileNotFoundException {
        ...
        jLabelNascimento = new JLabel("Data de Nascimento:");
        jTextFieldNascimento = new JTextField();
        jButtonNascimentoAS = new JButton("Alterar/Salvar nascimento");
        ...
        jLabelEmail = new ArrayList<MyLabel>();
        jTextFieldEmail = new ArrayList<MyTextField>();
        jButtonEmailAS = new ArrayList<MyButton>();

        jLabelDDD = new ArrayList<MyLabel>();
        jTextFieldDDD = new ArrayList<MyTextField>();
        jLabelTelefone = new ArrayList<MyLabel>();
        jTextFieldTelefone = new ArrayList<MyTextField>();
        jButtonDDDTelefones = new ArrayList<MyButton>();
        ...
        jLabelNascimento.setFont(new Font("DejaVu Sans", 0, 16));
        jLabelNascimento.setBackground(new Color(255, 255, 255));
        jLabelNascimento.setForeground(new Color(62, 62, 62));

        jTextFieldNascimento.setText(json.getString("nascimento"));
        ...
        /*
        jLabelEmail = new ArrayList<JLabel>("E-mail:"); // Transformar em um array ao final
        jTextFieldEmail = new ArrayList<JTextField>();
        jButtonEmailAS = new ArrayList<JButton>("Alterar/Salvar e-mail");
        */

        // E-mails
        int numEmail = 1; // existe pelo menos uma caixa de e-mail

        for (String name : JSONObject.getNames(json)) {
          if (name.substring(0, name.length() - 1).equals("email")) {
            numEmail++;
          }
        }

        for (k = 0; k < numEmail; k++) {
            int aux = k;

            jLabelEmail.add(new MyLabel("E-mail " + String.valueOf(k + 1) + ":"));
            jLabelEmail.get(aux).index = aux;
            jLabelEmail.get(aux).setFont(new Font("DejaVu Sans", 0, 16));
            jLabelEmail.get(aux).setBackground(new Color(255, 255, 255));
            jLabelEmail.get(aux).setForeground(new Color(62, 62, 62));

            if (k > 0)
              jTextFieldEmail.add(new MyTextField(json.getString("email" + String.valueOf(aux))));
            else
              jTextFieldEmail.add(new MyTextField(json.getString("email")));

            jTextFieldEmail.get(aux).index = aux;
            jTextFieldEmail.get(aux).setEditable(false);

            jButtonEmailAS.add(new MyButton("Alterar/Salvar e-mail"));
            jButtonEmailAS.get(aux).index = aux;
            jButtonEmailAS.get(aux).setBackground(new Color(62, 62, 62));
            jButtonEmailAS.get(aux).setForeground(new Color(255, 255, 255));
            jButtonEmailAS.get(aux).setFont(new Font("DejaVu Sans", 1, 11));
            jButtonEmailAS.get(aux).setFocusable(false);
            jButtonEmailAS.get(aux).setFocusPainted(false);
            jButtonEmailAS.get(aux).setBorder(BorderFactory.createEmptyBorder(1, 1, 1, 1));
            jButtonEmailAS.get(aux).addActionListener(new ActionListener() {
              int index = jButtonEmailAS.get(aux).index;

              @Override
              public void actionPerformed(ActionEvent evt) {
                if (jTextFieldEmail.get(index).isEditable()) {
                  jTextFieldEmail.get(index).setEditable(false);

                  json.put("email", jTextFieldEmail.get(index).getText().toLowerCase());
                  ...
                  jTextFieldEmail.get(index).setEditable(true);
                }
              }
            });

            jButtonEmailAS.get(aux).addMouseListener(new MouseAdapter() {
              int index = jButtonEmailAS.get(aux).index;

              @Override
              public void mouseEntered(MouseEvent evt) {
                jButtonEmailAS.get(index).setBackground(new Color(240, 240, 240));
                jButtonEmailAS.get(index).setForeground(new Color(62, 62, 62));
              }

              @Override
              public void mouseExited(MouseEvent evt) {
                jButtonEmailAS.get(index).setBackground(new Color(62, 62, 62));
                jButtonEmailAS.get(index).setForeground(new Color(255, 255, 255));
              }
            });
        }
        ...
        for (k = 0; k < numTelefones; k++) {
            int aux = k;

            jLabelDDD.add(new MyLabel("DDD " + String.valueOf(k + 1) + ":"));
            jTextFieldDDD.add(new MyTextField());
            jLabelTelefone.add(new MyLabel("Telefone " + String.valueOf(k + 1) + ":"));
            jTextFieldTelefone.add(new MyTextField());

            jLabelDDD.get(k).index = aux;
            jLabelDDD.get(k).setFont(new Font("DejaVu Sans", 0, 16));
            jLabelDDD.get(k).setBackground(new Color(255, 255, 255));
            jLabelDDD.get(k).setForeground(new Color(62, 62, 62));

            jTextFieldDDD.get(k).index = aux;
            jTextFieldDDD.get(k).setText(json.getString("DDD" + String.valueOf(k + 1)));
            jTextFieldDDD.get(k).setEditable(false);

            jLabelTelefone.get(k).index = aux;
            jLabelTelefone.get(k).setFont(new Font("DejaVu Sans", 0, 16));
            jLabelTelefone.get(k).setBackground(new Color(255, 255, 255));
            jLabelTelefone.get(k).setForeground(new Color(62, 62, 62));

            jTextFieldTelefone.get(k).index = aux;
            jTextFieldTelefone.get(k).setText(json.getString("telefone" + String.valueOf(k + 1)));
            jTextFieldTelefone.get(k).setEditable(false);

            jButtonDDDTelefones.add(new MyButton("Alterar/Salvar DDD e telefone"));
            jButtonDDDTelefones.get(k).index = aux;
            jButtonDDDTelefones.get(k).setBackground(new Color(62, 62, 62));
            jButtonDDDTelefones.get(k).setForeground(new Color(255, 255, 255));
            jButtonDDDTelefones.get(k).setFont(new Font("DejaVu Sans", 1, 11));
            jButtonDDDTelefones.get(k).setFocusable(false);
            jButtonDDDTelefones.get(k).setFocusPainted(false);
            jButtonDDDTelefones.get(k).setBorder(BorderFactory.createEmptyBorder(1, 1, 1, 1));
            jButtonDDDTelefones.get(k).addActionListener(new ActionListener() {
              @Override
              public void actionPerformed(ActionEvent evt) {
                int index = jButtonDDDTelefones.get(aux).index;

                if (jTextFieldDDD.get(index).isEditable()) {
                  jTextFieldDDD.get(index).setEditable(false);
                  jTextFieldTelefone.get(index).setEditable(false);

                  json.put("DDD" + String.valueOf(index + 1), jTextFieldDDD.get(index).getText());
                  json.put("telefone" + String.valueOf(index + 1), jTextFieldTelefone.get(index).getText());
                  ...
                  jTextFieldDDD.get(index).setEditable(true);
                  jTextFieldTelefone.get(index).setEditable(true);
                }
              }

            });
            jButtonDDDTelefones.get(k).addMouseListener(new MouseAdapter() {
              int index = jButtonDDDTelefones.get(aux).index;

              @Override
              public void mouseEntered(MouseEvent evt) {
                jButtonDDDTelefones.get(index).setBackground(new Color(240, 240, 240));
                jButtonDDDTelefones.get(index).setForeground(new Color(62, 62, 62));
              }

              @Override
              public void mouseExited(MouseEvent evt) {
                jButtonDDDTelefones.get(index).setBackground(new Color(62, 62, 62));
                jButtonDDDTelefones.get(index).setForeground(new Color(255, 255, 255));
              }
            });
        }
        ...
        jTextFieldNascimento.setText(json.getString("nascimento"));
        jTextFieldNascimento.getDocument().addDocumentListener(new DocumentListener() {
            String text = jTextFieldNascimento.getText();

            @Override
            public void changedUpdate(DocumentEvent evt) {
                text = jTextFieldNascimento.getText();
                ChangeJSON();
            }
                ...
                Runnable doInsert = new Runnable() {
                    ...
                    public void run() {
                        if (text.length() == 2 || text.length() == 5) {
                          text += "/";
                          jTextFieldNascimento.setText(text);
                        }
                    }
                };
            ...
            public void removing() {
                ...
                Runnable doRemove = new Runnable() {
                    @Override
                    public void run() {
                        if (text.length() == 2 || text.length() == 5) {
                ...
                SwingUtilities.invokeLater(doRemove);
            }

            public void ChangeJSON() {
                json.put("nascimento", text);
                try {
                    WriteJSON();
        ...
        jButtonNascimentoAS.setBackground(new Color(62, 62, 62));
        jButtonNascimentoAS.setForeground(new Color(255, 255, 255));
        jButtonNascimentoAS.setFont(new Font("DejaVu Sans", 1, 11));
        jButtonNascimentoAS.setFocusable(false);
        jButtonNascimentoAS.setFocusPainted(false);
        jButtonNascimentoAS.setBorder(BorderFactory.createEmptyBorder(1, 1, 1, 1));
        jButtonNascimentoAS.addActionListener(new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent evt) {
            if (jTextFieldNascimento.isEditable()) {
              jTextFieldNascimento.setEditable(false);

              json.put("nascimento", jTextFieldNascimento.getText());
              ...
              jTextFieldNascimento.setEditable(true);
            }
          }
        });
        jButtonNascimentoAS.addMouseListener(new MouseAdapter() {
          @Override
          public void mouseEntered(MouseEvent evt) {
            jButtonNascimentoAS.setBackground(new Color(240, 240, 240));
            jButtonNascimentoAS.setForeground(new Color(62, 62, 62));
          }
        ...
        // Campo de nascimento
        hGroup.addGroup(jPanelMainLayout.createSequentialGroup()
          .addComponent(jLabelNascimento, GroupLayout.PREFERRED_SIZE, 50 + 100, GroupLayout.PREFERRED_SIZE)
          .addGap(7)
          .addComponent(jTextFieldNascimento, GroupLayout.PREFERRED_SIZE, 294 - 100, GroupLayout.PREFERRED_SIZE)
          .addGap(15)
          .addComponent(jButtonNascimentoAS, GroupLayout.PREFERRED_SIZE, 175, GroupLayout.PREFERRED_SIZE)
        );
        vGroup.addGap(10);
        vGroup.addGroup(jPanelMainLayout.createParallelGroup()
          .addComponent(jLabelNascimento, GroupLayout.PREFERRED_SIZE, 25, GroupLayout.PREFERRED_SIZE)
          .addComponent(jTextFieldNascimento, GroupLayout.PREFERRED_SIZE, 25, GroupLayout.PREFERRED_SIZE)
          .addComponent(jButtonNascimentoAS, GroupLayout.PREFERRED_SIZE, 25, GroupLayout.PREFERRED_SIZE)
        );
        ...
        for (k = 0; k < numEmail; k++) {
            hGroup.addGroup(jPanelMainLayout.createSequentialGroup()
            .addComponent(jLabelEmail.get(k), GroupLayout.PREFERRED_SIZE, 50 + 15, GroupLayout.PREFERRED_SIZE)
            .addGap(7)
            .addComponent(jTextFieldEmail.get(k), GroupLayout.PREFERRED_SIZE, 294 - 15, GroupLayout.PREFERRED_SIZE)
            .addGap(15)
            .addComponent(jButtonEmailAS.get(k), GroupLayout.PREFERRED_SIZE, 175, GroupLayout.PREFERRED_SIZE)
            );
            vGroup.addGap(10);
            vGroup.addGroup(jPanelMainLayout.createParallelGroup()
            .addComponent(jLabelEmail.get(k), GroupLayout.PREFERRED_SIZE, 25, GroupLayout.PREFERRED_SIZE)
            .addComponent(jTextFieldEmail.get(k), GroupLayout.PREFERRED_SIZE, 25, GroupLayout.PREFERRED_SIZE)
            .addComponent(jButtonEmailAS.get(k), GroupLayout.PREFERRED_SIZE, 25, GroupLayout.PREFERRED_SIZE)
            );
        }
        ...
        for (k = 0; k < numTelefones; k++) {
            hGroup.addGroup(jPanelMainLayout.createSequentialGroup()
              .addComponent(jLabelDDD.get(k), GroupLayout.PREFERRED_SIZE, 50 + 5, GroupLayout.PREFERRED_SIZE)
              .addGap(7)
              .addComponent(jTextFieldDDD.get(k), GroupLayout.PREFERRED_SIZE, 50 - 5, GroupLayout.PREFERRED_SIZE)
              .addGap(12)
              .addComponent(jLabelTelefone.get(k), GroupLayout.PREFERRED_SIZE, 75 + 5, GroupLayout.PREFERRED_SIZE)
              .addGap(7)
              .addComponent(jTextFieldTelefone.get(k), GroupLayout.PREFERRED_SIZE, 150 - 5, GroupLayout.PREFERRED_SIZE)
              .addGap(15)
              .addComponent(jButtonDDDTelefones.get(k), GroupLayout.PREFERRED_SIZE, 175, GroupLayout.PREFERRED_SIZE)
            );
            vGroup.addGap(10);
            vGroup.addGroup(jPanelMainLayout.createParallelGroup()
              .addComponent(jLabelDDD.get(k), GroupLayout.PREFERRED_SIZE, 25, GroupLayout.PREFERRED_SIZE)
              .addComponent(jLabelDDD.get(k), GroupLayout.PREFERRED_SIZE, 25, GroupLayout.PREFERRED_SIZE)
              .addComponent(jTextFieldDDD.get(k), GroupLayout.PREFERRED_SIZE, 25, GroupLayout.PREFERRED_SIZE)
              .addComponent(jLabelTelefone.get(k), GroupLayout.PREFERRED_SIZE, 25, GroupLayout.PREFERRED_SIZE)
              .addComponent(jTextFieldTelefone.get(k), GroupLayout.PREFERRED_SIZE, 25, GroupLayout.PREFERRED_SIZE)
              .addComponent(jButtonDDDTelefones.get(k), GroupLayout.PREFERRED_SIZE, 25, GroupLayout.PREFERRED_SIZE)
            );
        }
        // Campo Cidade
        hGroup.addGroup(jPanelMainLayout.createSequentialGroup()
    ...
    private JLabel jLabelNascimento;
    private JTextField jTextFieldNascimento;
    private JButton jButtonNascimentoAS;
    ...
    private static class MyTextField extends JTextField {
      ...
      private MyTextField(String text) {
        this.setText(text);
      }
